...
#In the future, I want to see a directory containing all the functional files, which allows us to just import the whole directory instead of every signle file there. That will make short work of this. maybe when i'm back to the dev position, i will do that.


def run_current_status(current_status):
    #process the hashed returned current status here with all the if statements
    #name of files and functions must be identical for safety during development process
    #in local functional files, DO NOT return another function for the response; we do not want to load the RAM/call stack up with just function calls
    #read the home.py file, which is written 100% by #me, so that you understand how this system works. if you have questions, ask me. if you simply do not trust me, just copy my methods and don't even question it.
    #i have put everything yall did before the implementation of this system in a file called main_reserved.txt because they are no longer needed for operation. i will not delete them, i will let yall do that yourselves if you wish.
